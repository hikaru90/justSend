...
2. From a clean tree of this project (no `.env`, no `data/*.db`, no `useSend-legacy/`):
...
3. Archive or rename `hikaru90/justSend` on GitHub (Settings → Archive / Rename).
4. Run `gitleaks detect --source . --config .gitleaks.toml` on the new clone.
