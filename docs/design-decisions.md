...
### useSend as inspiration, not dependency

`useSend-legacy/` documents the prior Next.js + Prisma + Redis + Hono + tRPC shape. Owlery reimplements the domain on SvelteKit without carrying that runtime.
...
| Inbound email | Not implemented (was a useSend roadmap item) |
