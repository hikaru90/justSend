...
Emails are always light: the shell pins `color-scheme: light` in `<head>` and
the base CSS, so clients never auto-darken the output. Body and outer tables
set a `bgcolor` attribute for clients that do not support CSS
`background-color`.
...
```
parse → heal → normalize → tokens → preheader → serialize
        → fluidify → lint
```
