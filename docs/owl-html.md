...
runs a light-pinning override pass (`applyLightOverride`) that re-asserts every
inline light color under `@media (prefers-color-scheme: dark)` via stable
`owll-<data-owl-id>` classes, stamps `data-ogsc`/`data-ogsb` light values for
Outlook, and emits `u + .body` Gmail blend-mode rules — so the email renders
identically in Apple Mail, Outlook.com and Gmail dark mode. The body carries
`class="body"`, content sits inside `gmail-blend-screen` /
`gmail-blend-difference` wrapper divs, and gradients are pinned with
`background-image:linear-gradient(<color>,<color>)`. Body and outer tables set
a `bgcolor` attribute for clients that do not support CSS `background-color`.
...
> regenerated by the pass — do not author your own dark-mode variants.
