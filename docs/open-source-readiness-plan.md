...
| Git history | **Fresh `owlery` repo** with one clean initial commit | Avoid shipping private-dev artifacts (local DB, session tokens) via git history |
...
- 🔴 A local SQLite file with session tokens was committed in private history. **Wipe history** before making the repo public; keep `data/` gitignored.
...
- 🟠 Tracked junk: `.DS_Store`, `package.json.tmp`, `opencode.json` (listed in `.gitignore` yet still tracked), empty `patches/` directory.
- 🟠 Untracked but should be committed: `docs/`. Untracked and should stay out: `data/`.

### Identity / naming

- GitHub repo and package name are `owlery`; runtime default DB is `owlery.db`; API keys are prefixed `us_`; DKIM selector is `owlery`.
...
- [ ] **0.1** `.gitignore`: keep all of `data/` local (no DB / design-asset exceptions).
- [ ] **0.2** `git rm -r --cached data/` if anything under `data/` is still tracked.
- [ ] **0.3** Rotate local `AUTH_SECRET` — sessions were in private history, so treat old signing secrets as compromised.
- [ ] **0.4** *(Owner)* push the cleaned tree as the initial commit on `hikaru90/owlery`.
...
- [ ] **1.7** Canonical `owlery.db` everywhere (`.env.example`, compose, supervisor, docs).
...
- Owner-only actions on github.com: push the fresh history (0.4), flip public (6.2). Everything else is local/PR work.
