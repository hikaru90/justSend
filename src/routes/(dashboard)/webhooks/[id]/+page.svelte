...
<p class="mb-6 text-sm text-[hsl(var(--muted-foreground))]">
	{data.webhook.description ?? 'No description'}
</p>

<div class="mb-6 flex flex-wrap gap-2">
	<form method="POST" action="?/toggle" use:enhance>
		<Button type="submit" variant="outline">Toggle status</Button>
	</form>
	...
	<form
		...
		action="?/delete"
		use:enhance
		onsubmit={(e) => !confirm('Delete webhook?') && e.preventDefault()}
	>
...
<Card title="Recent calls">
	<table class="w-full text-sm">
		<thead class="border-b text-left">
			<tr
				><th class="p-2">Type</th><th class="p-2">Status</th><th class="p-2">Attempt</th><th
					class="p-2"
				></th></tr
			>
		</thead>
