...
export const actions: Actions = {
	create: async ({ request }) => {
		...
		const usesendUrl = String(form.get('usesendUrl') ?? '').trim();
		const sendingRateLimit = Number(form.get('sendingRateLimit') ?? 1);
		const transactionalQuota = Number(form.get('transactionalQuota') ?? 50);

		if (!region || !usesendUrl) return fail(400, { error: 'Region and URL required' });

		try {
			await createSesSetting({ region, usesendUrl, sendingRateLimit, transactionalQuota });
		} catch (e) {
