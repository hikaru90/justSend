<script lang="ts">
	...
	let previewTo = $state(data.userEmail ?? '');
	let sending = $state(false);
	let saving = $state(false);
	let deletePending = $state(false);
	let saveMessage = $state<string | null>(null);
	let templateName = $state(data.template.name);
	let testEmail = $state(data.studioSnapshot.testVariables?.email ?? 'alex@example.com');
	let testFirstName = $state(data.studioSnapshot.testVariables?.firstName ?? 'Alex');
	let testLastName = $state(data.studioSnapshot.testVariables?.lastName ?? 'River');
	let subject = $state(data.template.subject);
	let templatePrompt = $state(data.template.prompt ?? '');
	let preheader = $state(data.owlDoc?.preheader ?? '');
