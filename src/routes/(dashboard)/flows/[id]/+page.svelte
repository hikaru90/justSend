<script lang="ts">
	...
	let name = $state('');
	let status = $state<'draft' | 'active' | 'paused'>('draft');
	let contactBookId = $state('');
	...
	$effect.pre(() => {
		...
		name = flow.name;
		status = flow.status;
		contactBookId = flow.triggerConfig.contactBookId ?? '';
		nodes = structuredClone(flow.graph.nodes).map((n) => ({
			...n,
			type: n.type ?? 'default',
			data: { ...n.data },
		})) as Node[];
		edges = structuredClone(flow.graph.edges).map((e) => ({
			id: e.id,
			source: e.source,
			target: e.target,
