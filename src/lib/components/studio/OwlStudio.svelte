<script lang="ts">
	...
	let currentDoc = $state<OwlDoc>(emptyOwlDoc(''));
	...
	let uploadingSlotAsset = $state(false);
	let uploadSlotError = $state<string | null>(null);
	...
	let localDesignSections = $state<DesignSection[]>([]);
	...
	async function uploadSlotImage(file: File, slot: OwlSlot) {
		uploadSlotError = null;
		uploadingSlotAsset = true;
		try {
			const kind = slot.name.startsWith('logo') ? 'logo' : 'image';
			const asset = await uploadAsset(file, kind);
			if (asset?.id) {
				setSlot(slot.owlId, designAssetPath(asset.id));
			} else {
				uploadSlotError = 'Upload failed.';
			}
		} catch (e) {
			uploadSlotError = e instanceof Error ? e.message : 'Upload failed.';
		} finally {
			uploadingSlotAsset = false;
		}
	}
							...
							<label class="block space-y-1 text-xs">
								...
								<input
									...
									onchange={(e) => {
										...
										if (file) void uploadSlotImage(file, slot);
										e.currentTarget.value = '';
									}}
								/>
							</label>
							{#if uploadingSlotAsset}
								<p class="text-[10px] text-[hsl(var(--muted-foreground))]">Uploading…</p>
							{/if}
							{#if uploadSlotError}
								<p class="text-[10px] text-[hsl(var(--destructive))]">{uploadSlotError}</p>
							{/if}
