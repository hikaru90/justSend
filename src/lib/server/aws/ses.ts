...
const RESERVED_EMAIL_HEADER_PREFIXES = ['arc-', 'resent-', 'x-ses-', 'x-owlery-'];
...
export function buildHeaders({
	...
	if (emailId) {
		defaultHeaders['X-Owlery-Email-ID'] = emailId;
	}
...
export async function sendRawEmail({
	...
	const sesClient = getSesClient(region);
	console.log('[mail][ses] sendRawEmail ENTER', {
	...
	console.log('[mail][ses] sending SES SendEmailCommand', { emailId: emailId ?? null, region });
	const response = await sesClient.send(command);
	console.log('[mail][ses] SES responded', {
		emailId: emailId ?? null,
