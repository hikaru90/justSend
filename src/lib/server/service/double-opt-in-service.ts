...
export type DoiSendResult = { sent: boolean; reason?: string };
 ...
 * contact book has double opt-in disabled (no email sent). Throws on
 * validation / domain / SES errors so callers can surface the failure.
 */
export async function sendDoubleOptInConfirmationEmail({
	...
	teamId: number;
}): Promise<DoiSendResult> {
	console.log('[mail][doi] sendDoubleOptInConfirmationEmail ENTER', {
		contactId,
		contactBookId,
		teamId,
	});
	...
	console.log('[mail][doi] contact loaded', { contactId: contact.id, email: contact.email });
	...
	if (!contactBook || !contactBook.doubleOptInEnabled) {
		return { sent: false, reason: 'Double opt-in is disabled for this contact book' };
	}
	...
	console.log('[mail][doi] from resolved', { configuredFrom: configuredFrom ?? null, from });
	...
	try {
		...
		console.log('[mail][doi] html rendered ok', { length: html.length });
	} catch (error) {
		console.error('[mail][doi] html render FAILED, using fallback', error);
	...
	console.log('[mail][doi] validateDomainFromEmail ok', { from, teamId });

	console.log('[mail][doi] calling sendEmail', {
		teamId,
		to: contact.email,
		from,
		subject,
	...
	const doiEmail = await sendEmail({
		teamId,
		to: contact.email,
		from,
		subject,
	...
	console.log('[mail][doi] sendEmail returned', {
		emailId: doiEmail.id,
		latestStatus: doiEmail.latestStatus,
	});

	console.log(
		'[mail][doi] returning { sent: true } based on latestStatus =',
		doiEmail.latestStatus,
	);
	return { sent: true };
}
...
export async function confirmDoubleOptInSubscription({
	...
	// Enroll confirmed contact into automation flows
	const book = db
		.select({ teamId: contactBooks.teamId })
		.from(contactBooks)
		.where(eq(contactBooks.id, updated.contactBookId))
		.get();
	if (book) {
		...
		handleContactCreated({
			id: updated.id,
			email: updated.email,
			contactBookId: updated.contactBookId,
			teamId: book.teamId,
		});
	}
