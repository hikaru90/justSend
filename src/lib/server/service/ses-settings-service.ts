...
async function isValidCallbackUrl(url: string) {
	try {
...
async function registerConfigurationSet(setting: SesSetting): Promise<SesSetting> {
	...
	const configGeneral = `${setting.idPrefix}-${setting.region}-owlery-general`;
	...
	const configClick = `${setting.idPrefix}-${setting.region}-owlery-click`;
	...
	const configOpen = `${setting.idPrefix}-${setting.region}-owlery-open`;
	...
	const configFull = `${setting.idPrefix}-${setting.region}-owlery-full`;
...
export async function createSesSetting({
	...
	const validation = await isValidCallbackUrl(parsedUrl);
	...
	try {
		const topicName = `${idPrefix}-${region}-owlery`;
