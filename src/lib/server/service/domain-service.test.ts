...
describe('domain-service', () => {
	...
	describe('validateDomainFromEmail', () => {
		...
		it('throws when domain does not belong to team', async () => {
			const team = createTeam();
			await expect(
				validateDomainFromEmail('noreply@unknown.example.com', team.id)
			).rejects.toThrow('Domain: unknown.example.com of from email is wrong');
		});
		...
		it('throws when domain is not verified', async () => {
			...
			await expect(
				validateDomainFromEmail('noreply@pending.example.com', team.id)
		...
		it('parses display-name wrapped from addresses', async () => {
			...
			const result = await validateDomainFromEmail(
				'Team <noreply@verified.example.com>',
				team.id
			);
			expect(result.id).toBe(domain.id);
		});
