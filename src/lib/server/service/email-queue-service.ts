...
export function queueEmail(
	...
	console.log('[mail][queueEmail] ENTER', {
		emailId,
		teamId,
		region,
		transactional,
	...
	const jobId = enqueue(queue, payload, { jobId: emailId, delayMs });
	console.log('[mail][queueEmail] enqueue returned', { emailId, queue, jobId });
	return jobId;
}
...
export async function executeEmail(payload: unknown): Promise<void> {
	...
	console.log('[mail][execute] ENTER', {
		emailId,
		isBulk: isBulk ?? null,
		unsubUrl: unsubUrl ?? null,
	});
	if (!emailId) {
		console.log('[mail][execute] no emailId in payload, abort');
		return;
	}
	...
	if (!email) {
		console.log(
			'[mail][execute] !!! email row NOT FOUND for emailId — job will complete with no send',
			{ emailId },
		);
		return;
	}
	...
	console.log('[mail][execute] resolved', {
		emailId: email.id,
		domainId: email.domainId,
		region,
		sesTenantId: domain?.sesTenantId ?? null,
	});

	const setting = getSetting(region);
	if (!setting) {
		console.error('[mail][execute] !!! no SES setting for region — marking FAILED', {
			emailId: email.id,
			region,
		});
	...
	if (!configurationSetName) {
		console.error('[mail][execute] !!! no configuration set — marking FAILED', {
			emailId: email.id,
		});
	...
	console.log('[mail][execute] limit check', {
		emailId: email.id,
		isLimitReached: limitCheck.isLimitReached,
		reason: limitCheck.reason ?? null,
	});
	if (limitCheck.isLimitReached) {
		console.error('[mail][execute] !!! limit reached — marking FAILED', { emailId: email.id });
		db.insert(emailEvents)
			.values({
				id: cuid(),
				emailId: email.id,
	...
	try {
		console.log('[mail][execute] calling sendRawEmail', {
			emailId: email.id,
			region,
		...
		db.update(emails)
			...
			.where(eq(emails.id, email.id))
			.run();
		console.log('[mail][execute] email SENT', {
			emailId: email.id,
			sesMessageId: messageId ?? null,
		});
	} catch (error) {
		console.error('[mail][execute] sendRawEmail THREW — marking FAILED', {
			emailId: email.id,
			error,
		});
