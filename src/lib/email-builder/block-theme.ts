	...
	switch (block.type) {
		...
		case 'Divider':
			return {
				...
				data: {
					...block.data,
					props: { ...props, lineColor: theme.muted },
				},
			};
		default:
			return block;
...
export function themeEmptyDocument(
	...
	const isStock =
		(root.data.backdropColor ?? EMPTY_DOCUMENT.root.data.backdropColor) ===
			EMPTY_DOCUMENT.root.data.backdropColor &&
		(root.data.canvasColor ?? EMPTY_DOCUMENT.root.data.canvasColor) ===
			EMPTY_DOCUMENT.root.data.canvasColor &&
		(root.data.textColor ?? EMPTY_DOCUMENT.root.data.textColor) ===
			EMPTY_DOCUMENT.root.data.textColor;
